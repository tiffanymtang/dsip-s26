...
#> same number of rows as iris, i.e., 150 rows
```
...
iris |>
  group_by(Species) |>
  summarize(Sepal.Length.sd = sd(Sepal.Length))
```
...
#> rbind cannot handle data frames with different column names, so it will throw an error. bind_rows can handle data frames with different column names, and will fill in the missing values with NA.
```
...
#> bind_cols will automatically rename columns if they have the same name, while cbind will keep the column names the same
```
...
left_join(band_members, band_instruments2_messy, by = c("name" = "artist"))
```
...
```{r join-ex19}
band_instruments2_messy |> 
  distinct()
# or if we want to only look at duplicates in the artist column
band_instruments2_messy |> 
...
#> The boxplot layer is drawn first, followed by the violin layer. The violin plots are being plotted on top of the boxplots, which are hidden behind the violin plots currently.
```
...
#> facet_grid() creates a 2D grid of plots (defined by the inputted formula, here ~ Variable), as opposed to facet_wrap() which creates a 1D strip of plots, which is then wrapped into a 2D grid.
```
...
```{r ggplot-ex27}
# # this throws an error! There is no "default" aesthetic mapping that geom_boxplot() can pull from, so the error being thrown is because geom_boxplot() is missing the aesthetic mapping.
# ggplot(mtcars) +
#   geom_violin(
